...
Cos basis is particularly nice because we can quickly process distances using _non-uniform FFT_ codes, and reuse existing Python functions (`scipy.fft.dct`) for inter-conversion.

## Example

![](smc_rdf.jpg)

## Computing Coefficients from Raw Distances

For a frame with pair distances {rᵢ}:

$$
c_k = \sum_i \frac{1}{r_i^2} \cdot \cos(k \cdot \theta_i),
\quad \theta_i \in [0, \pi]
$$
...
- Uses **OAS shrinkage** for robust covariance estimation
...
Trajectory       │  NUFFT per pair │    128 cosine modes
 nvt_prod.xtc ──▶│  Frame-by-frame │───▶ per frame & pair
                 └─────────────────┘
                                               │
                 ┌─────────────────┐            ▼
...
$$
R_{ii} = \frac{\text{reg\_strength}}
{(\text{max\_frac} \cdot |p_i|)^2}
$$

| Feature | Purpose |
|---------|---------|
| λ (adaptive) | Interpolates between gradient descent and Gauss-Newton |
